...
func builtinWorkflowSpecs() []WorkflowSpec {
	return []WorkflowSpec{
		...
		{ID: "external_evidence", Version: "6.1", Domain: DomainExternalMCP, Goal: "Query authorized legacy external sources and analyze normalized redacted evidence.", TriggerIntents: []string{"external_source", "legacy_external", "normalized_evidence"}, ObjectTypes: []string{"external_source", "evidence"}, Risk: ToolRiskReadonly, ExposedCapabilities: []string{"list_mcp_sources", "get_mcp_source_schema", "test_mcp_connection", "query_mcp_source", "multi_query_mcp_sources", "analyze_mcp_evidence"}},
		{ID: MCPAggregationOnboardingWorkflowID, Version: "6.3", Domain: DomainExternalMCP, Goal: "Create and monitor a governed remote MCP onboarding job after explicit user authorization.", TriggerIntents: []string{"mcp_onboarding", "mcp_connect", "mcp_register", "onboard", "connect", "register"}, ObjectTypes: []string{"mcp_server", "remote_mcp_server", "mcp_onboarding_job"}, Risk: ToolRiskHigh, ExposedCapabilities: []string{"onboard_mcp_server", "get_mcp_onboarding_status"}},
		{ID: MCPAggregationClientAuthorizationWorkflowID, Version: "6.3", Domain: DomainExternalMCP, Goal: "Create a governed Client authorization for an existing published MCP service and return its one-time endpoint credential.", TriggerIntents: []string{"mcp_client_authorization", "mcp_client", "client_authorization", "authorize_client", "grant", "authorize"}, ObjectTypes: []string{"mcp_client", "mcp_grant", "mcp_server"}, Risk: ToolRiskHigh, ExposedCapabilities: []string{"authorize_mcp_client"}},
		{ID: MCPAggregationQueryWorkflowID, Version: "6.3", Domain: DomainExternalMCP, Goal: "Discover the authorized MCP catalog, query approved read-only tools, and inspect safe invocation metadata.", TriggerIntents: []string{"mcp", "mcp_catalog", "mcp_tool", "mcp_query", "mcp_invocation", "aggregation", "evidence"}, ObjectTypes: []string{"mcp_catalog", "mcp_tool", "mcp_invocation", "external_source", "evidence"}, Risk: ToolRiskReadonly, ExposedCapabilities: []string{"list_mcp_catalogs", "list_mcp_tools", "query_aggregated_mcp", "get_mcp_invocation"}},
		...
		{ID: agentGuardObservationWorkflowID, Version: "6.3", Domain: DomainAgentGuard, Goal: "Assess smart-agent posture and investigate bounded redacted evidence without changing runtime state.", TriggerIntents: []string{"agent_guard", "codex", "claude", "conversation", "prompt_injection", "jailbreak", "security", "risk"}, ObjectTypes: []string{"ai_agent", "agent_guard_posture", "agent_guard_scope", "conversation_session", "conversation_item", "finding", "analysis", "runtime_settings"}, Risk: ToolRiskReadonly, ExposedCapabilities: []string{
			"assess_agent_guard_posture", "investigate_agent_guard_scope", "list_agent_guard_evidence", "assess_agent_guard_configuration", "list_agent_guard_catalog", "get_agent_guard_analysis", "get_agent_guard_action", "query_agent_conversations", "get_agent_conversation_content", "analyze_agent_conversation", "get_agent_conversation_analysis", "get_agent_guard_runtime_settings",
		}},
		{ID: agentGuardControlWorkflowID, Version: "6.3", Domain: DomainAgentGuard, Goal: "Perform explicitly requested, approved Agent Guard collection, configuration, lifecycle, or containment actions.", TriggerIntents: []string{"collect", "freeze", "resume", "kill", "delete", "update", "contain", "control"}, ObjectTypes: []string{"host", "conversation_session", "execution_unit", "instance", "session", "runtime_settings", "action"}, Risk: ToolRiskHigh, ExposedCapabilities: []string{
			"collect_agent_conversations", "freeze_agent_guard_execution_unit", "resume_agent_guard_execution_unit", "kill_agent_guard_execution_unit", "kill_agent_guard_instance", "delete_agent_guard_behavior_sessions", "update_agent_guard_runtime_settings", "get_agent_guard_action",
		}},
	}
}
