...
func RegisterAssetTools(registry *assistant.ToolRegistry, deps AssetToolDeps) error {
	if err := registry.Register(&assistant.ToolSpec{
		...
		Description:        "查询资产概览，包括软件、应用、数据库、Web服务、AI Agent、LLM服务、MCP Server和待复核资产数量",
	...
	if err := registry.Register(&assistant.ToolSpec{
		...
		Description:        "查询应用资产和 AI 资产列表，支持关键字、主机、分类、置信度、复核状态和状态筛选",
		...
		ArgsSchema: map[string]interface{}{
			...
			"properties": map[string]interface{}{
				...
				"host_id":        map[string]interface{}{"type": "string", "description": "主机ID"},
				"category":       map[string]interface{}{"type": "string", "description": "应用分类，如 database/web_service/web_framework/web_site/llm_service/ai_agent/mcp_server"},
