...
PostgreSQL 容器首次启动时，会自动执行 `/docker-entrypoint-initdb.d/` 目录下的 SQL 脚本。系统将 `init.sql` 挂载到该目录，实现数据库的自动初始化。初始化脚本包含创建扩展、建表、建索引和创建触发器等所有操作（详见数据库设计文档 V2.2）。
