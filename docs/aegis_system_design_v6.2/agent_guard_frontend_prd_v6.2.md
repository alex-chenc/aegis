...
本 PRD 将 V6.2 智能体运行防护能力收敛为一个父导航和两个子页面：
...
以下 V4 原型基于当前项目列表/表格页面样式，外层只展示智能体及基本信息，
...
两个子菜单使用当前 `el-menu-item.is-active` 的蓝青渐变激活态。不得再把
...
两个主页面不新增第三个侧边栏子菜单。外层内容区仍只展示 Agent 基本信息；
...
1. 侧边栏只有“智能体防护”下的两个子菜单。
2. `/detection/agent-guard` 重定向事件页。
3. 两个子菜单激活态、面包屑和文档标题正确。
4. 深链刷新后能恢复详情与筛选。
5. 无权限路由按既有鉴权逻辑处理。
...
2. 全局导航中只出现“智能体防护”及其两个指定子菜单。
3. 两个子页外层只显示 Agent 和基本信息；点击 Agent 后才弹出全景和分析。
