...
- 已登录用户 2 小时没有鼠标、键盘、触摸、滚动或页面可见性恢复等操作时，前端必须自动清理本地认证状态并跳转 `/login`。
- 登录页和强制改密页不启动空闲计时。
...
- 已登录业务页 2 小时无操作后自动清理认证状态并跳转 `/login`。
...
- 后端 `Final Answer` 仍以 `attack_graph` + `conclusions` JSON 作为机器可解析载体，用于持久化、回写告警结论和生成溯源图。
- 前端不得把 `attack_graph` 原始 JSON 直接作为聊天正文展示；检测到结构化最终结果后，聊天区必须转换为中文可读摘要，至少包含分析完成状态、风险等级、图谱摘要、告警结论和处置建议。
- 前端必须继续把原始最终内容保存在会话状态中用于解析和恢复，但用户主视图只展示中文结论摘要与 `AttackGraph` 交互式溯源图。
- SSE 流式输出结构化 JSON 期间，聊天区应显示“正在整理最终结论与溯源图...”等明确状态，避免用户看到半截 JSON 或误认为是入库数据。
...
### 7.3 系统配置子页面拆分与图片模型配置

系统配置入口拆分为两个独立子页面，避免把模型密钥配置和 Agent 安装操作堆叠在同一页面：

| 子页面 | 路由 | 用途 |
|--------|------|------|
| 模型配置 | `/settings/models` | 配置文本 LLM 与图片模型 |
| Agent 安装 | `/settings/agent` | 获取 Agent 安装命令、服务器地址和端口信息 |

`/settings` 必须重定向到 `/settings/models`。侧边导航中的“系统配置”必须以子菜单形式展示“模型配置”和“Agent 安装”，保证两个操作域可以直接访问。
